...
Hydrogen is the most common element in the universe. It consists of one proton and one electron, and varies from zero, one, and two neutrons. The isotope of hydrogen with zero protons is called protium, and this is the most common. The isotope with one proton is called deuterium. The isotope with two protons is called tritium.

In my last post, I briefly described the energy production that result from hydrogen existing in the sun, and Nuclear Fusion is the process used to produce energy + Helium to bring light to all the darkness in our solar system. This reaction is favorable at the temperature of the sun and likely in future nuclear reactors. The feasibly of fusion in reactors has yet to be determined as there are countless problems to solve from both physics and engineering perspective.

Hydrogen can also be used as a fuel source in a completely different application chemically, where Hydrogen and Oxygen combust to form Water. Water is the most quintessential material required for life to exist, so the chemical reaction to produce water has interesting properties.
...
There are limits to the efficiencies as this all depends on the devices that we build to perform combustion.

If you use a combustion engine, unfortunately the efficiency is limited by the Carnot Cycle (https://en.wikipedia.org/wiki/Carnot_cycle).

However, electrochemical devices known as fuel cells can also be used. There are different types of fuel cells that exist and they are easily comparable to batteries.

The most commonly used Fuel Cell device today is known as the Proton Exchange Membrane Fuel Cell (PEMFC). This relies on the Hydrogen Oxidation Reaction (HOR) happening at the anode, the positively charged ionic Hydrogen passing through the ionically conductive and electronically insulation electrolytic membrane and the 2 or 4 electron reaction pathway to water at the cathode.

However, new technologies such as the Solid Oxide Fuel Cells (SOFC) have also been garnering growing attention and they are proving to be promising. This relies on the Hydrogen Oxidation Reaction (HOR) at the anode and the Oxygen Reduction Reaction (ORR) at the cathode, the negatively charged ionic Oxygen passing through the electrolyte membrane and the final reaction to produce water happening during the process.


The feasibilty of using Fuel Cells has shown to be promising in transport applications because of the infinite supply of Oxygen in the atmosphere and the fact that Hydrogen is also common in nature, unfortunately not so much in pure form. Hydrogen is generally found in water and hydrocarbon compounds, so the biggest key takeaway is that in order for Hydrogen energy to prove effective, then we must extract and produce Hydrogen in the most chemically and physically efficient, energy efficient, cost efficient ways.
